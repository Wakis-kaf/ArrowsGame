...
namespace CustomLitJson
{
    ...
    public class JsonMapper
    {
        ...
        private static bool PropCheck(PropertyInfo propertyInfo)
        {
            if (propertyInfo == null)
                return false;

            // 检查属性是否有get方法且该方法是公共的
            if( propertyInfo.GetGetMethod(nonPublic: false)!=null)
            {
                return propertyInfo.GetCustomAttributes(typeof(JsonIgnore), true).Length == 0;
            }
            //return true;
            //if (propertyInfo.IsPubliclyGettable())
            //{
            //    return propertyInfo.GetCustomAttributes(typeof(JsonIgnore), true).Length == 0;
            //}
            return false;
        }
        private static bool FieldCheck(FieldInfo fieldInfo)
        {
            //return true;
            if (!fieldInfo.IsPublic &&  fieldInfo.GetCustomAttributes(typeof(JsonSerializer), true).Length>0)
            {
                return true;
            }
            if (fieldInfo.IsPublic && fieldInfo.GetCustomAttributes(typeof(JsonIgnore), true).Length == 0)
            {
                return true;
            }
            return false;
        ...
        private static void RegisterBaseImporters()
        {
            ...
            importer = delegate (object input) {
                int.TryParse(input.ToString(), out int result);return result;            };
            RegisterImporter(base_importers_table, typeof(string),
                typeof(int), importer);
            ...
            int depth)
        {
            ...
            foreach (PropertyMetadata p_data in props)
            {
                if (p_data.IsField)
                {
                    if (!((FieldInfo) p_data.Info).IsPublic)
                    {
